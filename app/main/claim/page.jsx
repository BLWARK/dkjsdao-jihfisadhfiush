...
const Claim = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const taskData = await taskList(); // Memuat daftar tugas
        ...
        if (taskData?.totalTask && taskData?.userTasks) {
          // Kalkulasi persentase selesai tugas
          const percentage = (taskData.userTasks / taskData.totalTask) * 100;
          setTaskCompletionPercentage(percentage.toFixed(2)); // Simpan persentase di state
        }
  ...
  return (
    <div className="invite-sec w-full flex flex-col justify-start items-center h-[1200px] overflow-y-scroll bgs">
      ...
      <div className="subtitle w-full flex items-center justify-center mt-2 font-bold gap-3">
          <Player
            autoplay
            loop
            src="https://lottie.host/3d378620-2448-4d8d-b126-0f376683fa7f/byWcMYKcMV.json"
            style={{ height: '50px', width: '50px', }}
          />
          <p className='text-[24px]'>Earn More Coins</p>
        </div>
        <div className="wrap-total-balance bgs w-full flex flex-col justify-center items-center my-6 mt-6 py-5 rounded-xl gap-2">
          <p className="font-bold text-[14px] text-white">Your Points</p>
        ...
        {/* <div className="w-full flex flex-col justify-center items-center my-5 mt-5 gap-2">
        <div className="flex justify-center items-center w-full gap-1">
            <p className="font-bold text-[14px] text-white">Your Task Progress</p>
            <IoMdRefresh className="text-white text-[18px] cursor-pointer font-bold" onClick={refreshPage} /> 
          </div>
          
          <div className="progress-container mt-2">
            <div className="progress-bar" style={{ width: `${taskCompletionPercentage}%` }}></div>
            <div className="progress-percentage">{taskCompletionPercentage}%</div>
          </div>
        </div> */}

        {/* Render Checkpoint Section */}
        <div className="subtitle w-full text-[12px] text-left mt-8 font-bold">
        ...
        <div className="subtitle w-full text-[14px] text-left mt-14 font-bold">
          <p>Partnership Tasks</p>
        </div>
        <div className="all-task overflow-y-scroll gap-6">
